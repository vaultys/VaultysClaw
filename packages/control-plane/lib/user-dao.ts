...
export const UserDao = {
  ...
  getByDid(did: string): User | null {
    const db = getDb();
    return (
      (db.prepare("SELECT * FROM users WHERE did = ?").get(did) as User) ?? null
    );
  },
  ...
  create(did: string, publicKey: string | null, isOwner: boolean): User {
    const db = getDb();
    db.prepare(
      "INSERT INTO users (id, did, public_key, is_owner) VALUES (?, ?, ?, ?)"
    ).run(did, did, publicKey ?? null, isOwner ? 1 : 0);
  ...
  linkEntraIdentity(
    ...
    db.prepare("UPDATE users SET entra_id = ? WHERE id = ?").run(
      entraObjectId,
      userId
    );
  },
  ...
  getByEntraId(entraId: string): User | null {
    ...
    return (
      (db
        .prepare("SELECT * FROM users WHERE entra_id = ?")
        .get(entraId) as User) ?? null
    );
  },
  ...
  update(
    ...
    if (fields.reports_to !== undefined) {
      sets.push("reports_to = ?");
      values.push(fields.reports_to);
    }
  ...
  setAdmin(did: string, isAdmin: boolean): void {
    const db = getDb();
    db.prepare("UPDATE users SET is_admin = ? WHERE did = ?").run(
      isAdmin ? 1 : 0,
      did
    );
  },
  ...
  hasOwner(): boolean {
    const db = getDb();
    const row = db
      .prepare("SELECT COUNT(*) as count FROM users WHERE is_owner = 1")
      .get() as { count: number };
    return row.count > 0;
  },
  ...
  query(
    ...
    if (q) {
      conditions.push(
        "(LOWER(COALESCE(u.name,'')) LIKE ? OR LOWER(COALESCE(u.email,'')) LIKE ? OR COALESCE(u.did,'') LIKE ?)"
      );
    ...
    if (isAdmin !== undefined) {
      conditions.push(
        isAdmin
          ? "(u.is_admin = 1 OR u.is_owner = 1)"
          : "(u.is_admin = 0 AND u.is_owner = 0)"
      );
    }
    ...
    if (realm) {
      conditions.push(
        "EXISTS (SELECT 1 FROM user_realms ur JOIN realms r ON r.id = ur.realm_id WHERE ur.user_id = u.id AND (r.id = ? OR r.slug = ?))"
      );
