...
import { MARKER_TYPES, typeColor, type MapCluster, type MapMarker } from "./types";
...
export function ClusterSidebar({
  ...
  const types = Array.from(new Set([...MARKER_TYPES, ...Object.keys(cluster.byType)]));
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span
              ...
              style={{ background: typeColor(cluster.dominantType) }}
            >
              {cluster.count}
            </span>
            <span className="text-sm font-medium text-foreground">{cluster.count} markers</span>
          </div>
        ...
        <div className="pt-2 border-t border-neutral-200">
          ...
          <div className="space-y-1 text-xs text-foreground-500">
            {types
              .filter((t) => (cluster.byType[t] ?? 0) > 0)
              .map((type) => (
                <div key={type} className="flex items-center justify-between">
                  <span className="flex items-center gap-1 capitalize">
                    <span className="w-1.5 h-1.5 rounded-full" style={{ background: typeColor(type) }} />
                    {type}
                  </span>
                  <span className="text-foreground font-medium">{cluster.byType[type]}</span>
                </div>
              ))}
          </div>
        </div>
              ...
              >
                <div className="flex items-start gap-2">
                  <span
                    ...
                    style={{ background: typeColor(marker.type) + "20", color: typeColor(marker.type) }}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-foreground-400 capitalize text-[10px] mt-0.5">
                      ...
                      {marker.online !== undefined && (
                        <span className="ml-1">{marker.online ? "🟢" : "⚪"}</span>
                      )}
                    </div>
                  </div>
                </div>
