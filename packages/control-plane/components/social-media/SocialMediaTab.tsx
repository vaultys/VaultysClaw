...
function CronPreview({ cron }: { cron: string }) {
  ...
  if (parts.length !== 5) return <span className="text-red-500 text-xs">Invalid cron</span>;
...
export function SocialMediaTab({ realmId }: SocialMediaTabProps) {
  ...
  // Derived
  const xCred = credentials.find((c) => c.service === "x" && c.name === "session");
  ...
  async function saveXCredentials(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCredMsg({ type: "err", text: err.error ?? "Failed to save credentials." });
      }
    } finally {
      setSavingCred(false);
    }
  ...
  async function deleteXCredential() {
    if (!confirm("Delete X credentials? The agent will no longer be able to post.")) return;
  ...
  async function saveSchedule(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json() as { error?: string };
        setScheduleMsg({ type: "err", text: err.error ?? "Failed to save schedule." });
      }
    } finally {
  ...
  async function postNow(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPostMsg({
          type: "ok",
          text: data.tweetUrl ? `Posted! ${data.tweetUrl}` : "Post dispatched to agent.",
        });
        setPostText("");
      } else {
        const err = await res.json() as { error?: string };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-vc-surface border border-vc-border rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {hasXCred && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800 font-medium flex items-center gap-1">
        ...
        {hasXCred ? (
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl text-xs text-amber-800 dark:text-amber-300">
              <Info className="w-4 h-4 shrink-0 mt-0.5" />
              <div className="space-y-1">
                <p className="font-medium">One-time browser session setup required</p>
                <p>
                  The agent posts via browser automation. Ask the agent to run{" "}
                  <code className="bg-amber-100 dark:bg-amber-900/40 px-1 py-0.5 rounded font-mono">setup_x_session</code>{" "}
                  once. It will open a browser window — log in to X and the session will be saved automatically.
                </p>
              </div>
            </div>
          ...
          <form onSubmit={saveXCredentials} className="space-y-3">
            ...
            {credMsg && (
              <p className={`text-xs flex items-center gap-1.5 ${credMsg.type === "ok" ? "text-emerald-600" : "text-red-500"}`}>
                {credMsg.type === "ok" ? <CheckCircle2 className="w-3.5 h-3.5" /> : <AlertTriangle className="w-3.5 h-3.5" />}
                {credMsg.text}
              </p>
            )}
            <div className="flex justify-end">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium transition-colors"
              >
                {savingCred ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Key className="w-3.5 h-3.5" />}
      ...
      <div className="bg-vc-surface border border-vc-border rounded-2xl p-5 space-y-4">
        ...
        <form onSubmit={saveSchedule} className="space-y-3">
          ...
          {scheduleMsg && (
            <p className={`text-xs flex items-center gap-1.5 ${scheduleMsg.type === "ok" ? "text-emerald-600" : "text-red-500"}`}>
              {scheduleMsg.type === "ok" ? <CheckCircle2 className="w-3.5 h-3.5" /> : <AlertTriangle className="w-3.5 h-3.5" />}
              {scheduleMsg.text}
            </p>
          )}
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium transition-colors"
            >
              {savingSchedule ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Clock className="w-3.5 h-3.5" />}
      ...
      <div className="bg-vc-surface border border-vc-border rounded-2xl p-5 space-y-4">
        ...
        <form onSubmit={postNow} className="space-y-3">
          ...
          {postMsg && (
            <p className={`text-xs flex items-center gap-1.5 break-all ${postMsg.type === "ok" ? "text-emerald-600" : "text-red-500"}`}>
              {postMsg.type === "ok" ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <AlertTriangle className="w-3.5 h-3.5 shrink-0" />}
              {postMsg.text}
            </p>
          )}
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white text-sm font-medium transition-colors"
            >
