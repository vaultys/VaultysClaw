...
interface TreePos { x: number; depth: number }
...
interface LayoutResult {
  ...
  grantEdges:  Array<{ userId: string; agentId: string; caps: string[]; isDelegation: boolean }>;
...
function computeLayout(data: GraphData): LayoutResult {
  ...
  for (const e of data.edges) {
    ...
    const child = e.source, parent = e.target;
  ...
  function getWidth(id: string): number {
    ...
    const w = children.length === 0 ? 1 : children.reduce((s, c) => s + getWidth(c), 0);
    widths.set(id, w);
    return w;
  }
  ...
  const agentsSorted = [...agentNodes].sort((a, b) => {
    const ax = agentUserXs.has(a.id)
      ? agentUserXs.get(a.id)!.reduce((s, v) => s + v, 0) / agentUserXs.get(a.id)!.length
      : Infinity;
    const bx = agentUserXs.has(b.id)
      ? agentUserXs.get(b.id)!.reduce((s, v) => s + v, 0) / agentUserXs.get(b.id)!.length
      : Infinity;
    return ax - bx;
  });
  ...
  return {
    positions, parentMap, childMap, userMap,
    agentNodes: agentsSorted, agentPositions,
    grantEdges, totalWidth, maxDepth,
  };
}
...
function nodeCenter(pos: TreePos) {
  return { cx: pos.x + NODE_W / 2, cy: pos.depth * (NODE_H + V_GAP) + NODE_H / 2 };
}
    ...
    <div style={{ height, position: "relative" }}>
      ...
      >
        ...
        <defs>
          ...
          <marker id="arrow-deleg" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
            <path d="M0,0 L0,6 L6,3 z" fill={C.delegLine} />
          </marker>
          <marker id="arrow-hier" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
        ...
        {Array.from(positions.entries()).map(([id, pos]) => {
          ...
          return (
            <path
              ...
              stroke={hovered === id || hovered === parentId ? C.edgeHighlight : C.edgeLine}
        ...
        {grantEdges.map((ge, i) => {
          ...
          return (
            <path
              ...
              markerEnd={ge.isDelegation ? "url(#arrow-deleg)" : "url(#arrow-grant)"}
        ...
        {agentNodes.length > 0 && showAgents && (
          <line
            ...
            stroke={C.separator} strokeWidth="1"
          />
        )}
        ...
        {Array.from(positions.entries()).map(([id, pos]) => {
          ...
          const x = pos.x, y = pos.depth * (NODE_H + V_GAP);

          // Avatar fill: use a hash of the label for consistent colour
          const avatarHue = Array.from(label).reduce((acc, c) => acc + c.charCodeAt(0), 0) % 360;
          ...
          return (
            ...
            >
              ...
              <rect x="0" y="14" width="3" height={NODE_H - 28} rx="1.5" fill={rs.border} />
              ...
              <text
                x="74" y={NODE_H / 2 - 7}
                fill={C.nameText} fontSize="13" fontWeight="600"
                fontFamily="system-ui, sans-serif"
              >{truncate(label, 16)}</text>

              {/* Role badge */}
              <rect
                x="74" y={NODE_H / 2 + 5}
                width={role.length * 7 + 16} height="17" rx="8.5"
                fill={rs.bg} stroke={rs.border} strokeWidth="0.5"
              />
              <text
                x={74 + (role.length * 7 + 16) / 2} y={NODE_H / 2 + 17}
                textAnchor="middle" fill={rs.text}
        ...
        {agentNodes.map((node) => {
          ...
          return (
            ...
            >
              <rect
                width={AGENT_W} height={AGENT_H} rx="10"
                fill={online ? C.agentBg : C.agentBgOffline}
                stroke={isHov ? (online ? "#22c55e" : "#94a3b8") : (online ? C.agentBorderOn : C.agentBorderOff)}
                strokeWidth={isHov ? 2 : 1.5}
                style={{ transition: "stroke 0.15s" }}
              />
              {/* Icon placeholder — small bot rect */}
              <rect x="10" y={AGENT_H / 2 - 13} width="26" height="26" rx="7"
                fill={online ? "#052e16" : "#0f172a"} />
              {/* Bot eyes */}
              <circle cx="20" cy={AGENT_H / 2 - 1} r="2.5" fill={online ? "#6ee7b7" : "#475569"} />
              <circle cx="27" cy={AGENT_H / 2 - 1} r="2.5" fill={online ? "#6ee7b7" : "#475569"} />
              <rect x="16" y={AGENT_H / 2 + 4} width="15" height="2" rx="1" fill={online ? "#6ee7b7" : "#475569"} />
              ...
              >{truncate(label, 14)}</text>

              {/* Online dot */}
              <circle cx={AGENT_W - 12} cy={AGENT_H / 2} r="5"
...
function LegendSVG({ x, y }: { x: number; y: number }) {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <rect width="110" height={items.length * 18 + 12} rx="8"
        fill="#0f172a" stroke="#1e293b" strokeWidth="1" opacity="0.9" />
      {items.map((item, i) => (
        <g key={item.label} transform={`translate(10, ${14 + i * 18})`}>
          <line x1="0" y1="0" x2="20" y2="0"
            stroke={item.color} strokeWidth="2"
            strokeDasharray={item.dash || undefined} />
          <text x="26" y="4" fill="#94a3b8" fontSize="10" fontFamily="system-ui, sans-serif">
            {item.label}
          </text>
        </g>
      ))}
