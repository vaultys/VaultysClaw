...
const steps: { phase: BastionPhase; label: string; description: string }[] = [
  {
    phase: "connect",
    label: "Initializing",
    description: "Establishing secure connection with browser device…",
  },
  {
    phase: "waiting",
    label: "Waiting",
    description: "Waiting for your wallet to respond…",
  },
  {
    phase: "authenticate",
    label: "Authenticating",
    description: "Verifying your browser device securely…",
  },
  {
    phase: "success",
    label: "Connected",
    description: "Browser device authenticated!",
  },
  {
    phase: "failure",
    label: "Failed",
  ...
  return (
    <div className="flex flex-col items-center gap-6 text-center">
      ...
      <div className="w-full max-w-xs bg-background-200 rounded-full h-1.5">
        <div
          className={`h-1.5 rounded-full transition-all duration-500 ${isError ? "bg-danger-500" : "bg-primary-500"}`}
          style={{
            width:
              bastionPhase === "connect"
                ? "33%"
                : bastionPhase === "waiting"
                  ? "66%"
                  : bastionPhase === "authenticate"
                    ? "90%"
                    : "100%",
          }}
        />
