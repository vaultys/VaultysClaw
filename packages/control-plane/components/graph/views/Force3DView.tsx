...
import type { GraphData, GraphNode, GraphNodeType, GraphEdgeType } from "@vaultysclaw/shared";
...
export default function Force3DView({ data, height, onNodeClick }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([import("react-force-graph-3d"), import("three")]).then(([fgMod, threeMod]) => {
...
function LegendItem({ color, label, shape, icon }: { color: string; label: string; shape?: "circle" | "line"; icon?: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
      {icon ? (
        <span className="inline-flex items-center justify-center w-4 h-4 rounded-full" style={{ backgroundColor: color }}>{icon}</span>
      ) : shape === "line" ? (
...
function IconBot() {
  return <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 8V4H8"/><rect x="6" y="8" width="12" height="12" rx="2"/><circle cx="9.5" cy="13.5" r=".5" fill="white"/><circle cx="14.5" cy="13.5" r=".5" fill="white"/></svg>;
}
