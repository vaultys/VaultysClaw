...
import { DETAIL_ZOOM_LEVEL, typeColor, typeOnlineColor, type RenderPoint } from "./types";

/** Build a data-URI SVG pin (circle + tail, optional centered text). */
export function buildPinSvg({ color, size, text = "" }: { color: string; size: number; text?: string }) {
  const r = size / 2 - 2;
  const cx = size / 2;
  const cy = r + 2;
  const h = size + 12;
...
export function buildPointStyles(rp: RenderPoint, zoom: number, isDark: boolean): OlStyle[] {
  const labelColor = isDark ? "#e2e8f0" : "#334155";
  const detailColor = isDark ? "#94a3b8" : "#64748b";

  if (rp.kind === "marker") {
    const { marker } = rp;
    const color = marker.online !== false ? typeOnlineColor(marker.type) : typeColor(marker.type);
    ...
    if (zoom >= 3) {
      styles.push(
        new OlStyle({
          text: new OlText({
            text: marker.label.length > 20 ? marker.label.slice(0, 18) + "…" : marker.label,
    ...
    if (zoom >= DETAIL_ZOOM_LEVEL) {
      const detail = `${marker.type}${marker.online !== undefined ? (marker.online ? " · online" : " · offline") : ""}`;
      styles.push(
        new OlStyle({
          text: new OlText({
            text: detail,
  ...
  const { cluster } = rp;
  const color = typeColor(cluster.dominantType);
