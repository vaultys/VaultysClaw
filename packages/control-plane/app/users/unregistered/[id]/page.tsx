    ...
    <div className="p-6 w-full max-w-7xl mx-auto space-y-0">
      ...
      <div className="border border-neutral-200 rounded-xl overflow-hidden bg-background-100">
        ...
        <div className="p-6">
          ...
          {activeTab === "realms" && <RealmsTab realms={user.realms} />}
        </div>
      </div>
...
function DangerZone({ id, userName }: { id: string; userName: string | null }) {
  ...
  const handleRemove = async () => {
    if (!confirm(`Remove ${userName ?? "this user"}? This cannot be undone.`))
      return;
    setRemoving(true);
  ...
  return (
    <div className="mt-6 bg-background-100 border border-danger-200 rounded-2xl p-6">
      ...
      <p className="text-foreground-500 text-sm mb-4">
        Removing this provisioned user will delete their record. They will need
        to be re-synced from Entra to be re-invited.
      </p>
      <button
        onClick={handleRemove}
        disabled={removing}
        className="flex items-center gap-2 bg-danger-600 hover:bg-danger-500 disabled:opacity-50 text-white font-medium text-sm px-4 py-2.5 rounded-xl transition-colors"
      >
