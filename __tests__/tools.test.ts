...
describe("Shell tool — edge cases", () => {
  ...
  it("should truncate stdout that exceeds the size limit", async () => {
    ...
    const result = await shellTool.tool.execute!(
      { command: "node", args: ["-e", "process.stdout.write('A'.repeat(70000))"], timeoutMs: 10000 },
      {} as any,
    );
    expect(result.exitCode).toBe(0);
...
describe("HTTP request tool — extended", () => {
  ...
  it("should return body for text/html responses", async () => {
    ...
    const result = await httpRequestTool.tool.execute!(
      { url: "https://example.com/", method: "GET", headers: {}, timeoutMs: 5000 },
      {} as any,
    );
    expect(result.status).toBe(200);
  ...
  it("should describe binary responses without reading the body", async () => {
    ...
    const result = await httpRequestTool.tool.execute!(
      { url: "https://example.com/logo.png", method: "GET", headers: {}, timeoutMs: 5000 },
      {} as any,
    );
    expect(result.status).toBe(200);
  ...
  it("should truncate large response bodies", async () => {
    ...
    const result = await httpRequestTool.tool.execute!(
      { url: "https://example.com/big", method: "GET", headers: {}, timeoutMs: 5000 },
      {} as any,
    );
  ...
  it("should return response headers", async () => {
    ...
    const result = await httpRequestTool.tool.execute!(
      { url: "https://api.example.com/", method: "GET", headers: {}, timeoutMs: 5000 },
      {} as any,
    );
  ...
  it("should return 4xx status codes without throwing", async () => {
    ...
    const result = await httpRequestTool.tool.execute!(
      { url: "https://api.example.com/missing", method: "GET", headers: {}, timeoutMs: 5000 },
      {} as any,
    );
...
describe("File ops tools — edge cases", () => {
  let tmpDir: string;
  let tools: Awaited<ReturnType<typeof import("../packages/agent-controller/src/tools/file-ops").createFileTools>>;
  ...
  it("should return an error (not throw) for a non-existent file", async () => {
    const result = await readTool.tool.execute!({ path: "does-not-exist.txt" }, {} as any);
    expect(result.error).toBeDefined();
    expect(result.error).toContain("not found");
  });

  it("should return an error (not throw) when listing a non-existent directory", async () => {
    const result = await listTool.tool.execute!({ path: "no-such-dir" }, {} as any);
    expect(result.error).toBeDefined();
  });
  ...
  it("should overwrite an existing file", async () => {
    await writeTool.tool.execute!({ path: "overwrite.txt", content: "first" }, {} as any);
    await writeTool.tool.execute!({ path: "overwrite.txt", content: "second" }, {} as any);
    const result = await readTool.tool.execute!({ path: "overwrite.txt" }, {} as any);
    expect(result.content).toBe("second");
  });

  it("should write and read back empty content", async () => {
    await writeTool.tool.execute!({ path: "empty.txt", content: "" }, {} as any);
    const result = await readTool.tool.execute!({ path: "empty.txt" }, {} as any);
  ...
  it("should truncate large file reads", async () => {
    ...
    await writeTool.tool.execute!({ path: "big.txt", content: bigContent }, {} as any);
  ...
  it("should report file size in the read result", async () => {
    await writeTool.tool.execute!({ path: "sized.txt", content: "hello" }, {} as any);
    const result = await readTool.tool.execute!({ path: "sized.txt" }, {} as any);
    expect(result.size).toBe(5);
  });

  it("should list files with their type", async () => {
    await writeTool.tool.execute!({ path: "file.txt", content: "x" }, {} as any);
  ...
  it("should report bytes written", async () => {
    const result = await writeTool.tool.execute!({ path: "bytes.txt", content: "hello" }, {} as any);
...
describe("Code runner tool — edge cases", () => {
  ...
  it("should capture console.warn and console.error output", async () => {
    const { codeRunnerTool } = await import("../packages/agent-controller/src/tools/code-runner");
    const result = await codeRunnerTool.tool.execute!(
      { code: "console.warn('a warning'); console.error('an error'); 'done'", timeoutMs: 5000 },
      {} as any,
    );
  ...
  it("should include partial console output even when execution throws", async () => {
    const { codeRunnerTool } = await import("../packages/agent-controller/src/tools/code-runner");
    const result = await codeRunnerTool.tool.execute!(
      { code: "console.log('before'); throw new Error('boom'); console.log('after')", timeoutMs: 5000 },
      {} as any,
    );
    expect(result.error).toBeDefined();
