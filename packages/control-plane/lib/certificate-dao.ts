...
export const CertificateDao = {
  create(input: CertificateInput): Certificate {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, input.key, input.registration, input.connection, input.register, input.data, input.status, input.metadata ?? null);
  ...
  update(id: string, patch: Partial<Certificate>): void {
    ...
    db.prepare(`UPDATE certificates SET ${set} WHERE id = ?`).run(...values, id);
  },

  getByRegistration(token: string): Certificate | null {
    const db = getDb();
    return (db.prepare("SELECT * FROM certificates WHERE registration = ?").get(token) as Certificate) ?? null;
  },

  getByConnection(token: string): Certificate | null {
    const db = getDb();
    return (db.prepare("SELECT * FROM certificates WHERE connection = ?").get(token) as Certificate) ?? null;
  },

  getByKey(key: string): Certificate | null {
    const db = getDb();
    return (db.prepare("SELECT * FROM certificates WHERE key = ?").get(key) as Certificate) ?? null;
  },

  consumeCertificate(connectionToken: string): { count: number } {
    const db = getDb();
    const result = db.prepare("DELETE FROM certificates WHERE connection = ? AND status = 2").run(connectionToken);
