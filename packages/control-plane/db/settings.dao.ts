...
export async function setDoclingConfig(cfg: { url: string; enabled: boolean; sourceEndpoint?: string; fileEndpoint?: string; locationLat?: number; locationLon?: number; locationLabel?: string }) {
  ...
  if (cfg.locationLat != null) await SettingsDAO.set("docling_location_lat", String(cfg.locationLat));
  if (cfg.locationLon != null) await SettingsDAO.set("docling_location_lon", String(cfg.locationLon));
  if (cfg.locationLabel != null) await SettingsDAO.set("docling_location_label", cfg.locationLabel);
}
...
export async function setStorageConfig(cfg: { storageType?: string; filesystemDir?: string; s3Enabled?: boolean; s3Region?: string; s3Bucket?: string; s3Endpoint?: string; s3AccessKeyId?: string; s3SecretAccessKey?: string; locationLat?: number; locationLon?: number; locationLabel?: string }) {
  ...
  if (cfg.locationLat != null) await SettingsDAO.set("s3_location_lat", String(cfg.locationLat));
  if (cfg.locationLon != null) await SettingsDAO.set("s3_location_lon", String(cfg.locationLon));
  if (cfg.locationLabel != null) await SettingsDAO.set("s3_location_label", cfg.locationLabel);
}
