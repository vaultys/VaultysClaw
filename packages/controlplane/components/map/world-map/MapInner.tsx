...
export function MapInner({
  ...
  return (
    <div className={`relative flex select-none ${className ?? ""}`} style={{ height }}>
      ...
      {selectedCluster && (
        <ClusterSidebar cluster={selectedCluster} onClose={() => setSelectedCluster(null)} />
      )}
      ...
      {editingMarker && onSaveLocation && (
        <LocationEditor
          current={{ lat: editingMarker.lat, lon: editingMarker.lon, label: editingMarker.label }}
