...
export default function LoginPage() {
  return (
    <div className="h-screen bg-gradient-to-br from-gray-950 via-indigo-950 to-gray-950 relative overflow-hidden">
      {/* Floating header */}
      <header className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-8 py-5 bg-gradient-to-b from-black/30 to-transparent pointer-events-none">
        <div className="flex items-center gap-3 pointer-events-auto animate-fade-in-up">
          <div className="w-9 h-9 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-600/30 text-lg leading-none">
            🦞
          </div>
          <span className="text-xl font-bold tracking-tight text-white">VaultysClaw</span>
        </div>
        <div
          className="flex items-center gap-1.5 bg-emerald-900/30 border border-emerald-700/50 rounded-full px-3 py-1 pointer-events-auto animate-fade-in-up"
          style={{ animationDelay: "100ms" }}
        >
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
          <span className="text-xs font-bold text-emerald-400">Secure by design</span>
        </div>
      </header>
