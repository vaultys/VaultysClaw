...
describe("MemoryStore", async () => {
  ...
  const { MemoryStore } = await import("../packages/agent-controller/src/memory/store");
  ...
  it("saves and retrieves a memory by FTS search", () => {
    ...
    store.save({ type: "fact", content: "The user prefers TypeScript over JavaScript", tags: ["language"] });
  ...
  it("saves with importance clamped to [0,1]", () => {
    const store = new MemoryStore();
    store.save({ type: "preference", content: "Deploy on Fridays", importance: 2.5 });
  ...
  it("stores and parses tags correctly", () => {
    const store = new MemoryStore();
    store.save({ type: "procedure", content: "Run pnpm build before deploy", tags: ["deploy", "ci"] });
  ...
  it("deletes a memory", () => {
    const store = new MemoryStore();
    const id = store.save({ type: "fact", content: "Temporary fact to delete" });
    store.delete(id);
  ...
  it("parseTags handles invalid JSON gracefully", () => {
    const fakeRow = { id: "x", type: "fact" as const, content: "test", tags: "not-json", importance: 0.5, access_count: 0, last_accessed: null, created_at: "" };
    expect(MemoryStore.parseTags(fakeRow)).toEqual([]);
  });

  it("search bumps access_count", () => {
    const store = new MemoryStore();
    store.save({ type: "fact", content: "bumping access count for this memory" });
    // Search twice
...
describe("MemoryRetriever", async () => {
  const dbModule = await import("../packages/agent-controller/src/db");
  const { MemoryStore } = await import("../packages/agent-controller/src/memory/store");
  const { MemoryRetriever } = await import("../packages/agent-controller/src/memory/retriever");

  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vaultysclaw-retriever-test-"));
    dbModule.initDb(tmpDir);
  });
  ...
  it("formats retrieved memories as a context block", () => {
    ...
    store.save({ type: "fact", content: "The database uses PostgreSQL version 15" });
  ...
  it("includes high-importance recent memories even without keyword match", () => {
    ...
    store.save({ type: "preference", content: "Always use strict TypeScript", importance: 0.95 });
  ...
  it("respects maxChars budget", () => {
    ...
    // Save many memories
    for (let i = 0; i < 20; i++) {
      store.save({ type: "fact", content: `Memory number ${i} about deployment pipeline configuration` });
    }
  ...
  it("returns empty string when all memories are below minImportance threshold", () => {
    const store = new MemoryStore();
    const retriever = new MemoryRetriever(store, { minImportance: 0.99, searchLimit: 0 });

    store.save({ type: "fact", content: "low importance memory", importance: 0.1 });
...
describe("Tool usage logging", async () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vaultysclaw-tool-log-test-"));
    dbModule.initDb(tmpDir);
  });
  ...
  it("logToolUsage persists a log entry", () => {
    ...
    const rows = dbModule.getDb().query(
      "SELECT * FROM tool_usage_log WHERE tool_name = 'shell'"
    ).all() as any[];
  ...
  it("logToolUsage records failure", () => {
    dbModule.logToolUsage("http_request", { url: "http://fail.invalid" }, false, 300);
    const rows = dbModule.getDb().query(
      "SELECT * FROM tool_usage_log WHERE tool_name = 'http_request'"
    ).all() as any[];
  ...
  it("multiple log entries accumulate", () => {
    ...
    const rows = dbModule.getDb().query(
      "SELECT * FROM tool_usage_log WHERE tool_name = 'code_runner'"
    ).all() as any[];
    expect(rows.length).toBe(2);
  });
});
