  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${data.role === "owner" ? "bg-yellow-600" :
          data.role === "admin" ? "bg-blue-600" :
            data.role === "manager" ? "bg-indigo-600" :
              data.role === "operator" ? "bg-emerald-600" :
                "bg-slate-600"
          }`}>
          {initials(data.label)}
        </div>
...
function computeLayout(data: GraphData, currentUserId?: string): LayoutResult {
  ...
  function getWidth(id: string): number {
    ...
    const children = (childMap.get(id) ?? []).filter(c => visibleUserIds.has(c));
...
export default function OrgChartFlowView({ data, height, onNodeClick, currentUserId }: Props) {
  const { nodes: layoutNodes, edges: layoutEdges } = useMemo(() => computeLayout(data, currentUserId), [data, currentUserId]);
