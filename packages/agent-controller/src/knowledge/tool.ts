  ...
  return createTool({
    ...
    inputSchema: z.object({
      ...
      sourceId: z.string().optional()
        .describe('Restrict search to a specific knowledge source ID (optional)'),
    }),
    ...
    execute: async ({ query, topK, sourceId }) => {
      ...
      if (!llmConfig) {
        return { results: [], totalSources: 0, message: 'LLM not configured — cannot generate embeddings for search.' };
      }
      ...
      if (readySources.length === 0) {
        return { results: [], totalSources: 0, message: 'No knowledge bases are ready. Ask an admin to sync a knowledge source.' };
      }
      ...
      try {
        ...
        logger.info({ query, found: results.length }, 'Knowledge search executed');
        ...
        return { results: [], totalSources: readySources.length, message: `Search failed: ${msg}` };
      }
    },
  });
}
