...
function buildPointStyles(
  ...
  if (rp.kind === "marker") {
    ...
    const size = Math.max(
      20,
      Math.min(34, 30 / Math.sqrt(Math.max(1, zoom / 2)))
    );
    const h = size + 12;
    const styles: OlStyle[] = [
      new OlStyle({
        image: new OlIcon({
          src: buildPinSvg({ color, size }),
          anchor: [0.5, (h - 1) / h],
  ...
  const size = Math.max(
    24,
    Math.min(42, 36 / Math.sqrt(Math.max(1, zoom / 2)))
  );
  const h = size + 12;
  ...
  return [
    new OlStyle({
      image: new OlIcon({
        src: buildPinSvg({ color, size, text }),
        anchor: [0.5, (h - 1) / h],
        anchorXUnits: "fraction",
        anchorYUnits: "fraction",
        size: [size, h],
...
function MapInner({
  ...
  useEffect(() => {
    ...
    const tileLayer = new TileLayer({
      source: makeXYZ(isDarkRef.current, retina),
    });
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span
                  ...
                  style={{
                    background: TYPE_COLOR[selectedCluster.dominantType],
                  }}
                >
                  {selectedCluster.count}
                </span>
                <span className="text-sm font-medium text-foreground">
                  {selectedCluster.count} markers
                </span>
              </div>
              <div className="text-xs text-foreground-500 space-y-1 ml-8">
                <div>{selectedCluster.onlineCount} online</div>
                <div className="text-foreground-400">
                  {selectedCluster.lat.toFixed(4)},{" "}
                  {selectedCluster.lon.toFixed(4)}
                </div>
              </div>
            </div>
            <div className="pt-2 border-t border-neutral-200">
              ...
              <div className="space-y-1 text-xs text-foreground-500">
                {(["agent", "user", "docling", "s3"] as const)
                  .filter((t) => selectedCluster.byType[t] > 0)
                  .map((type) => (
                    <div
                      key={type}
          ...
          {tooltip.kind === "marker" && tooltip.marker && (
            <>
              ...
              <div className="text-xs text-foreground-500 space-y-0.5">
                ...
                <div className="text-foreground-400">
                  {tooltip.marker.lat.toFixed(4)},{" "}
                  {tooltip.marker.lon.toFixed(4)}
                </div>
            ...
            <>
              <div className="flex items-center gap-2 mb-1.5">
                <span
                  ...
                  style={{
                    background: TYPE_COLOR[tooltip.cluster.dominantType],
                  }}
                >
              ...
              <div className="text-xs text-foreground-500 space-y-0.5">
                ...
                <div className="text-foreground-400">
                  {tooltip.cluster.lat.toFixed(4)},{" "}
                  {tooltip.cluster.lon.toFixed(4)}
                </div>
      ...
      <div className="bg-background border border-neutral-200 rounded-2xl shadow-xl w-full max-w-sm p-5">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <button
            onClick={handleSave}
            disabled={
              loading || (mode === "city" ? !city.trim() : !lat || !lon)
            }
