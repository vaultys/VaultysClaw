...
function makeToolSet(tools: Record<string, ReturnType<typeof makeTool>>): ToolSet {
...
describe("parseTextToolCall", () => {
  ...
  it("parses {name, parameters} format", () => {
    const text = JSON.stringify({ name: "file_list", parameters: { path: "/tmp" } });
  ...
  it("parses {tool, arguments} format", () => {
    const text = JSON.stringify({ tool: "file_read", arguments: { path: "hello.txt" } });
    const result = parseTextToolCall(text, toolNames);
    expect(result).toEqual({ toolName: "file_read", args: { path: "hello.txt" } });
  });
...
describe("executeToolCall", () => {
  it("executes a known tool and returns result", async () => {
    const tools = makeToolSet({
      file_list: makeTool("file_list", { entries: [{ name: "hello.txt", type: "file" }] }),
    });
    const result = await executeToolCall("file_list", { path: "." }, tools);
    expect(result.tool).toBe("file_list");
    expect(result.result).toEqual({ entries: [{ name: "hello.txt", type: "file" }] });
  });

  it("passes args to the tool's execute function", async () => {
    const mockExecute = vi.fn().mockResolvedValue({ content: "hello" });
    const tools = makeToolSet({
      file_read: { description: "read", parameters: {}, execute: mockExecute } as any,
    });
    await executeToolCall("file_read", { path: "test.txt" }, tools);
    expect(mockExecute).toHaveBeenCalledWith({ path: "test.txt" }, expect.objectContaining({ toolCallId: expect.any(String) }));
  });
  ...
  it("returns error result when tool throws", async () => {
    const tools = makeToolSet({
      bad_tool: { description: "bad", parameters: {}, execute: vi.fn().mockRejectedValue(new Error("permission denied")) } as any,
    });
...
describe("resolveToolResults", () => {
  it("Case 1: collects existing tool results from steps", async () => {
    ...
    const output = {
      ...
      steps: [{
        toolResults: [
          { toolName: "file_list", output: { entries: [{ name: "a.txt", type: "file" }] } },
        ],
      }],
    };
    ...
    expect(parsed[0].result).toEqual({ entries: [{ name: "a.txt", type: "file" }] });
  });
  ...
  it("Case 1: uses result field when output is absent", async () => {
    ...
    const output = {
      text: "",
      steps: [{
        toolResults: [{ toolName: "shell", result: { exitCode: 0, stdout: "hello" } }],
      }],
    };
  ...
  it("Case 2: executes tool manually when tool call has no results", async () => {
    const mockExecute = vi.fn().mockResolvedValue({ entries: [{ name: "b.txt", type: "file" }] });
    const tools = makeToolSet({
      file_list: { description: "list", parameters: {}, execute: mockExecute } as any,
    });
    const output = {
      text: "",
    ...
    expect(result[0].result).toEqual({ entries: [{ name: "b.txt", type: "file" }] });
  });
  ...
  it("Case 2: passes available args when present", async () => {
    ...
    const output = {
      text: "",
      steps: [{ toolCalls: [{ toolName: "file_list", args: { path: "/tmp" } }] }],
    };
    await resolveToolResults(output, tools);
    expect(mockExecute).toHaveBeenCalledWith({ path: "/tmp" }, expect.anything());
  });

  it("Case 3: executes tool call from text when no steps", async () => {
    const mockExecute = vi.fn().mockResolvedValue({ entries: [{ name: "c.txt", type: "file" }] });
    const tools = makeToolSet({
      file_list: { description: "list", parameters: {}, execute: mockExecute } as any,
    });
    const output = {
    ...
    const result = JSON.parse(await resolveToolResults(output, tools));
    expect(result[0].tool).toBe("file_list");
    expect(result[0].result).toEqual({ entries: [{ name: "c.txt", type: "file" }] });
  });
  ...
  it("Case 1 takes priority over Case 2 when both present", async () => {
    ...
    const output = {
      ...
      steps: [{
        ...
        toolResults: [{ toolName: "file_list", result: { entries: [{ name: "existing.txt", type: "file" }] } }],
      }],
    };
    const result = JSON.parse(await resolveToolResults(output, tools));
