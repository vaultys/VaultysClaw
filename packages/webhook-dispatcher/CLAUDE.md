...
here, out of band. Delivery is **best-effort** — nothing is persisted; BullMQ's
retry policy (set on the producer job: 5 attempts, exponential backoff) handles
transient failures.
...
- **`src/index.ts`** — entry point. Creates the BullMQ `Worker`, loads active
  `Webhook` rows, filters by subscribed event type, and delivers to each.
