import { AgentSummary, ChatSessionSummary } from "@/lib/api-types";
...
export class AgentsApi extends BaseApi {
  list(params?: { page?: number; pageSize?: number; realm?: string }) {
    ...
    return this.get<{ agents: Agent[]; total?: number }>(`/api/agents${qs ? `?${qs}` : ""}`);
  }
  ...
  update(did: string, data: Partial<Pick<Agent, "name" | "description" | "metadata">>) {
  ...
  getLlmConfig(did: string) {
    return this.get<AgentLlmConfig>(`/api/agents/${did}/llm-config`);
  }
  ...
  getChatSessions(did: string) {
    return this.get<{ sessions: ChatSessionSummary[] }>(`/api/agents/${did}/chat-sessions`);
  }

  // Skills
  getSkills(did: string) {
    return this.get<{ skills: AgentSkillOverride[] }>(`/api/agents/${did}/skills`);
  }

  updateSkills(did: string, skills: AgentSkillOverride[]) {
    return this.patch<{ skills: AgentSkillOverride[] }>(`/api/agents/${did}/skills`, { skills });
  }
  ...
  getTokenUsage(did: string, params?: { from?: string; to?: string }) {
    ...
    const qs = query.toString();
    return this.get<AgentTokenUsage>(`/api/agents/${did}/token-usage${qs ? `?${qs}` : ""}`);
  }
  ...
  addPeer(did: string, data: { peerDid: string; capabilities: string[]; expiresAt?: string }) {
  ...
  // Tasks
  sendTask(did: string, data: { intent: string; payload?: Record<string, unknown> }) {
