...
interface Props {
  ...
  /** Number of models granted to this realm (needed to check if provision is possible) */
  modelCount: number;
  onRefresh: () => void;
}

export function RealmLiteLLMKeyCard({
  realmId,
  routerKey,
  litellmConfigured,
  modelCount,
  ...
  return (
    <>
      <ConfirmModal
        open={showRevoke}
        title="Revoke realm router key"
        message="The realm's LiteLLM virtual key will be removed. Agents that were using it will keep their last pushed config until reconfigured."
      ...
      <div className="bg-background-100 border border-neutral-200 rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-foreground-500">
                Shared virtual key scoped to this realm — agents use it for model routing
              </p>
            </div>
          </div>
        ...
        {editing ? (
          <div className="p-4 space-y-4">
            <p className="text-xs text-foreground-500">
              {modelCount === 0
                ? "No models are granted to this realm yet. Grant access to models first via the Model Registry."
                : `A virtual key will be provisioned in LiteLLM covering all ${modelCount} model(s) currently granted to this realm.`}
            </p>
