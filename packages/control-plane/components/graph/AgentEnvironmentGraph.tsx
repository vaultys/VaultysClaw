...
import {
  Bot, Server, Globe, FolderOpen, Brain, Users,
  BookOpen, ShieldOff, ShieldCheck, Mail, Plug,
...
const PAL: Record<string, { accent: string; tint: string; glow: string }> = {
  agent: { accent: "#6366f1", tint: "rgba(99,102,241,.07)", glow: "rgba(99,102,241,.18)" },
  cp: { accent: "#22d3ee", tint: "rgba(34,211,238,.06)", glow: "rgba(34,211,238,.14)" },
  llm: { accent: "#fbbf24", tint: "rgba(251,191,36,.06)", glow: "rgba(251,191,36,.14)" },
  internet: { accent: "#4ade80", tint: "rgba(74,222,128,.06)", glow: "rgba(74,222,128,.14)" },
  files: { accent: "#60a5fa", tint: "rgba(96,165,250,.06)", glow: "rgba(96,165,250,.14)" },
  knowledge: { accent: "#c084fc", tint: "rgba(192,132,252,.07)", glow: "rgba(192,132,252,.15)" },
  peer: { accent: "#38bdf8", tint: "rgba(56,189,248,.05)", glow: "rgba(56,189,248,.12)" },
  peerOffline: { accent: "#78716c", tint: "rgba(120,113,108,.04)", glow: "rgba(120,113,108,.08)" },
  mail: { accent: "#a78bfa", tint: "rgba(167,139,250,.06)", glow: "rgba(167,139,250,.14)" },
  api: { accent: "#2dd4bf", tint: "rgba(45,212,191,.06)", glow: "rgba(45,212,191,.13)" },
  denied: { accent: "#f87171", tint: "rgba(248,113,113,.06)", glow: "rgba(248,113,113,.14)" },
};
...
type NodeKind = "agent" | "cp" | "llm" | "internet" | "files" | "knowledge" | "peer" | "mail" | "api";
...
function EnvironmentNode({ data }: NodeProps<NodeData>) {
  ...
  return (
    ...
    >
      ...
      <Handle id="left-src" type="source" position={Position.Left} style={{ opacity: 0 }} />
      <Handle id="left-tgt" type="target" position={Position.Left} style={{ opacity: 0 }} />
      <Handle id="right-src" type="source" position={Position.Right} style={{ opacity: 0 }} />
      <Handle id="right-tgt" type="target" position={Position.Right} style={{ opacity: 0 }} />
      <Handle id="top-src" type="source" position={Position.Top} style={{ opacity: 0 }} />
      <Handle id="top-tgt" type="target" position={Position.Top} style={{ opacity: 0 }} />
      <Handle id="bottom-src" type="source" position={Position.Bottom} style={{ opacity: 0 }} />
      <Handle id="bottom-tgt" type="target" position={Position.Bottom} style={{ opacity: 0 }} />

      {/* Header row — reversed for right-aligned peer column */}
      <div style={{
        position: "relative", display: "flex", alignItems: "center",
        flexDirection: ra ? "row-reverse" : "row", gap: "6px", marginBottom: "3px"
      }}>
        {denied
          ? <ShieldOff size={13} color={pal.accent} style={{ flexShrink: 0 }} />
          : <Icon size={13} style={{ color: pal.accent, flexShrink: 0 } as React.CSSProperties} />}
        <span style={{ fontWeight: 600, fontSize: "12px", color: CV.text, flex: 1 }}>
          {data.label}
        </span>
        {data.badge && (
          <span style={{
      ...
      {data.sublabel && (
        <div style={{
          position: "relative", fontSize: "10px", color: CV.muted,
      ...
      {data.domains && data.domains.length > 0 && (
        <ul style={{ position: "relative", marginTop: "3px", paddingLeft: 0, listStyle: "none" }}>
          {data.domains.slice(0, 4).map((d) => (
            <li key={d} style={{ fontSize: "9px", color: pal.accent, fontFamily: "monospace" }}>
              · {d}
            </li>
          ))}
      ...
      {data.docCount !== undefined && (
        <div style={{ position: "relative", fontSize: "10px", color: pal.accent, marginTop: "2px" }}>
      ...
      {data.allowed && data.expiry !== undefined && (
        <div style={{
          position: "relative", fontSize: "9px", color: CV.subtle, marginTop: "3px",
          display: "flex", alignItems: "center",
          flexDirection: ra ? "row-reverse" : "row", gap: "3px",
        }}>
      ...
      {denied && (
        <div style={{
          position: "relative", fontSize: "9px", color: pal.accent,
...
function makeEdge(
  ...
  return {
    ...
    markerEnd: opts.markerEnd !== false
...
export default function AgentEnvironmentGraph({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const policiesJson = policiesRes.ok ? await policiesRes.json() : { policies: [] };
        const knowledgeJson = knowledgeRes.ok ? await knowledgeRes.json() : { sources: [] };
        setData({
          agents: (agentsJson.agents ?? []).filter((a: AgentSummary) => a.id !== agentId),
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    function addNode(node: Node<NodeData>) { pos[node.id] = node.position; nodes.push(node); }

    // Normal edge: handle sides derived from relative position
    function edge(
      id: string, src: string, tgt: string, label: string, color: string,
      opts: { animated?: boolean; dashed?: boolean; markerEnd?: boolean } = {},
    ) {
    ...
    function edgeH(
      ...
      const srcX = pos[src].x, tgtX = pos[tgt].x;
      ...
      edges.push({
        ...
        label, animated: opts.animated ?? true,
        style: { stroke: color, strokeWidth: 1.8, strokeDasharray: opts.dashed ? "5,4" : undefined },
        labelStyle: { fill: color, fontSize: 10, fontWeight: 600 },
        labelBgStyle: { fill: "transparent" },
    ...
    for (const p of data.policies) {
      for (const cap of p.capabilities) {
        if (!(cap in capExpiry)) {
          ...
          capExpiry[cap] = capExpiry[cap]! < p.expiresAt ? capExpiry[cap] : p.expiresAt;
        } else {
    ...
    const agentY = TOP_PAD + (peerCount > 0
      ? Math.round((peerCount - 1) * ROW_GAP / 2)
      : 0);
    ...
    sortedPeers.forEach((peer, i) => {
      addNode({
        id: `peer-${peer.id}`, type: "env",
        position: { x: PEER_COL_X, y: TOP_PAD + i * ROW_GAP },
        data: { kind: "peer", label: peer.name, badge: peer.online ? "online" : "offline",
                offline: !peer.online, rightAlign: true },
      });
    });

    // ── Agent ─────────────────────────────────────────────────────────────────
    addNode({
      id: "agent", type: "env", position: { x: COL.agent, y: agentY },
    ...
    sortedPeers.forEach((peer) => {
      edgeH(`e-peer-${peer.id}`, "agent", `peer-${peer.id}`,
        "",
    ...
    addNode({
      id: "cp", type: "env", position: { x: COL.agent, y: cpY },
      data: { kind: "cp", label: "Control Plane",
              sublabel: transport === "peerjs" ? "WebRTC / PeerJS" : "WebSocket" },
    });
    edge("e-cp-agent", "agent", "cp",
      transport === "peerjs" ? "WebRTC" : "WebSocket", EDGE_COLORS.transport);

    // ── LLM ───────────────────────────────────────────────────────────────────
    let rightRow = 0;
    if (llm) {
      addNode({
        id: "llm", type: "env", position: { x: COL.right, y: rightY0 + rightRow * ROW_GAP },
    ...
    if (hasInternet) {
      const internetExpiry = expiry("internet_access") ?? expiry("browser_control");
      addNode({
        id: "internet", type: "env",
        position: { x: COL.right, y: rightY0 + rightRow * ROW_GAP },
        data: {
          kind: "internet", label: "Internet", expiry: internetExpiry,
          sublabel: allowedDomains.length > 0
            ? `${allowedDomains.length} domain${allowedDomains.length !== 1 ? "s" : ""} allowed`
            : "unrestricted",
          domains: allowedDomains,
        },
      });
      edge("e-agent-internet", "agent", "internet",
        allowedDomains.length > 0 ? "filtered" : "open",
        EDGE_COLORS.internet, { animated: true });
      rightRow++;
    }
    ...
    if (hasFiles) {
      addNode({
        id: "files", type: "env",
        position: { x: COL.right, y: rightY0 + rightRow * ROW_GAP },
        data: { kind: "files", label: "File System",
                expiry: expiry("file_access"), sublabel: "workspace/" },
      });
      edge("e-agent-files", "agent", "files", "read / write",
        EDGE_COLORS.files, { animated: true });
      rightRow++;
    }
    ...
    if (hasMail) {
      addNode({
        id: "mail", type: "env",
        position: { x: COL.right, y: rightY0 + rightRow * ROW_GAP },
        data: { kind: "mail", label: "Mail",
                expiry: expiry("mail_send"), sublabel: "SMTP / send" },
      });
      edge("e-agent-mail", "agent", "mail", "send",
        EDGE_COLORS.mail, { animated: true });
      rightRow++;
    }
    ...
    if (hasApi) {
      addNode({
        id: "api", type: "env",
        position: { x: COL.right, y: rightY0 + rightRow * ROW_GAP },
        data: { kind: "api", label: "API Calls",
                expiry: expiry("api_call"), sublabel: "HTTP / external" },
      });
      edge("e-agent-api", "agent", "api", "call",
        EDGE_COLORS.api, { animated: true });
      rightRow++;
    }
    ...
    if (hasKnowledge) {
      ...
      data.knowledge.forEach((ks, i) => {
        ...
        addNode({
          ...
          data: { kind: "knowledge", label: ks.name, sublabel: ks.source_type,
                  docCount: ks.doc_count, badge: ks.status,
                  expiry: expiry("knowledge_search") },
        });
        edge(`e-agent-ks-${ks.id}`, "agent", nodeId, "RAG",
    ...
    return { nodes, edges };
  }, [data, agentId, agentName, transport, online, reportedLlm, storedLlm, capabilities]);
  ...
  if (err) {
    return (
      <div className="flex items-center gap-2 h-[520px] justify-center text-sm text-red-400">
