...
cleanup() {
  ...
  for cname in "${OBS_GRAFANA_CONTAINER:-}" "${OBS_COLLECTOR_CONTAINER:-}" "${OBS_PROMETHEUS_CONTAINER:-}" "${OBS_TEMPO_CONTAINER:-}" \
               "${LITELLM_CONTAINER:-}" "${DOCLING_CONTAINER:-}" "${MINIO_CONTAINER:-}" "${REDIS_CONTAINER:-}" "${PG_CONTAINER:-}"; do
    [[ -z "$cname" ]] && continue
...
# Redis — backs the BullMQ notification queue + per-user pub/sub for live
# notifications. Host port 6380 avoids clashing with a local Redis on 6379.
REDIS_CONTAINER="vc-demo-redis"
REDIS_PORT="6380"
...
export DATABASE_URL NEXTAUTH_SECRET LITELLM_MASTER_KEY REDIS_URL
...
if $FRESH; then
  ...
  docker rm -f "${PG_CONTAINER}" "${MINIO_CONTAINER}" "${DOCLING_CONTAINER}" "${LITELLM_CONTAINER}" "${REDIS_CONTAINER}" \
...
# =============================================================================
# Step 9b — Redis (BullMQ notification queue + pub/sub)
# =============================================================================
step "Redis"
...
if docker inspect "$REDIS_CONTAINER" &>/dev/null 2>&1; then
  if [[ "$(docker inspect -f '{{.State.Running}}' "$REDIS_CONTAINER")" != "true" ]]; then
    ...
    log "$REDIS_CONTAINER is already running."
  fi
else
  log "Creating $REDIS_CONTAINER on port ${REDIS_PORT}…"
  docker run -d \
...
# =============================================================================
# Step 11b — Notifier (consumes the notification queue, delivers in-app/email/push)
# =============================================================================
step "Notifier"

# The notifier runs as a standalone Node process (like the control plane). It
# imports @vaultysclaw/shared from its built dist (node --import tsx resolves
# workspace deps via the "import" condition), so build shared (+policy) first.
...
NOTIFIER_PID=$!
echo "$NOTIFIER_PID" >> "$PIDS_FILE"
log "Notifier started (PID $NOTIFIER_PID) → $LOG_DIR/notifier.log"
...
echo   "║  Redis:          127.0.0.1:${REDIS_PORT}  (notifications queue + pub/sub)"
echo   "║"
echo   "║  Logs:"
echo   "║    Control plane: $LOG_DIR/control-plane.log"
echo   "║    Notifier:      $LOG_DIR/notifier.log"
