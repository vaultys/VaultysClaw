...
pnpm simulator people            # mint the human population and (re)assign ownership
pnpm simulator stats             # what the control plane currently holds
pnpm simulator reset             # delete simulated Actors, their people, and their identities
pnpm simulator admin             # mint an admin human + export its VaultysID (see above)
pnpm simulator --help
```

Useful flags: `--actors` / `--agents` (fleet size), `--humans` (the population that owns it),
`--rate` (ramp), `--reconnect-rate`, `--duration`, `--auto-approve`, `--status-refresh <ms>`,
`--url`, `--data-dir`.

## The fleet belongs to somebody

A fleet of 7,000 Actors owned by nobody is accurate about the database and useless as a picture of
an estate, because the question an admin actually arrives with is *whose agent is this*. It also
left `/admin/graph` drawing a field of unconnected dots and the actor detail page's "owner" row
permanently empty. So `run` (and `approve`, and `people`) now mints a population as well as a
fleet, and points every Actor at the person it belongs to via `Actor.ownerDid`.

```
pnpm simulator people                  # idempotent: safe to re-run, cheap when nothing changed
pnpm simulator run --humans 0          # opt out — a pure load test wants no population
pnpm simulator run --humans 300        # or size it yourself
```

**They are real identities, and they do not connect.** Each person is a VaultysId generated by the
same `loadOrCreateIdentity` the fleet uses, persisted at `<data-dir>/human-NNNNN/identity` in the
same format, holding a real control-plane-signed `portal_access` certificate. But `human` is the one
kind onboarded through *login* rather than the WS registration handshake
(`controlplane/lib/actor-kinds.ts`), so driving one over `ActorRuntime` would be simulating a path
that does not exist. They are written straight into the ledger — the same deliberate back door
`simulator admin` uses, and `grants.ts` is now the one place that does it for both.

Five things are load-bearing:

- **The count scales with the fleet** (`--humans`, default a fifth of `--actors` + `--agents`).
  Neither extreme shows anything: one person per Actor is a list, not an org, and ten people owning
  seven thousand things is not an org either. Around five owned Actors per person — a laptop, the
  machine a sensor runs on, a coding agent, an MCP server or two — is the shape of a real estate and
  the density at which the graph is still readable.
- **Assignment is by hash of the DID**, like `locations.ts`, so a given agent has the same owner on
  every run. Approval order is *not* stable — it is whatever order the pending queue came back in —
  so anything keyed on position would redraw the graph between runs.
- **Ownership is skewed, not uniform.** `hash % humanCount` gives everybody very nearly the same
  number of Actors, which no real estate looks like. `ownership.ts` raises the hash to a power so
  most people hold three to five things and the busiest holds about twenty. The exponent is tuned by
  what it produces: much higher and one person owns a hundred Actors, which reads as a bug in the
  generator rather than as a power user.
- **A `proxy` has no owner**, and that is a deliberate `Persona.ownedByHuman` decision rather than
  an oversight — the same judgement `locations.ts` makes about the map. A laptop, a sensor's machine
  and a coding agent all have somebody whose they are. An interception point sitting in front of
  many people's traffic does not, and naming one of them its owner would be a claim the real
  deployment never makes.
- **Assignment is a separate pass, not a field set during approval.** Approval only walks the
  *pending* queue, so a fleet approved by an earlier run — the normal case from the second run on —
  would never get an owner. A pass over everything also covers a fleet approved before this existed,
  which is the same problem `locations.ts` solved less completely by re-applying on update.

### Why the people have names and the fleet has prefixes

Every other database operation here is scoped by name prefix (`openclaw-`, `sensor-`, …) so a
developer's real agent in the same queue is never touched by a load test. That cannot work for
people: an ownership demo whose every owner is called "human-00042" answers "whose agent is this?"
with another question. So the population is scoped by a reserved email domain instead —
`@sim.invalid`, which RFC 2606 guarantees can never resolve — which leaves the *display* name free
to be an actual name, and is a stricter filter than a prefix besides. `reset` deletes on that
domain; the `simulator admin` human, which has no such address, survives it. The agents survive it
too: `ownerDid` is `onDelete: SetNull`, so removing the population unlinks the fleet rather than
deleting it.

The names come from disjoint French first/last pools (matching `locations.ts`'s regional network —
a map of Bpifrance offices whose owners are all called Smith reads as two demos stapled together).
Two details that are less arbitrary than they look, both found by looking at the output:

- **The surname strides by `FIRST_NAMES.length + 1`**, not once per pass through the first names.
  Ownership skews toward low indices, so the natural enumeration made every heavy owner on the graph
  a Martin. The stride visits every (first, last) pair exactly once before repeating only because it
  is coprime with the surname count — which is what the test pins.
- **The pools share no name.** Thomas, Vincent and Nicolas are common in both, and leaving them in
  both produced "Thomas Thomas". Substituting a surname fixes it; patching the output afterwards
  does not, because the pair it would move to is one another index legitimately owns.
...
`__tests__/` covers the pure logic — the fleet-composition mix, the report rendering, and the
ownership distribution. The last one is worth the space: `db.ts` writes whatever
