...
export default function Login({ onLogin }: Props) {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      const { qrUrl: url, sessionId, agentDid: aDid } = await r.json() as ConnectResponse;
      ...
      pollTimer = setInterval(async () => {
        try {
          ...
          const { status, did } = await sr.json() as { status: string; did?: string };
    ...
    <div className="flex items-center justify-center min-h-full bg-canvas">
      <div className="bg-canvas-subtle border border-border rounded-xl p-8 w-[360px] max-w-[95vw] flex flex-col items-center gap-5">
        ...
        <div className="text-xs text-center min-h-[32px] flex items-center justify-center px-2">
          {phase === "starting" && (
            <span className="text-info">Starting authentication session…</span>
          )}
          {phase === "scanning" && (
            <span className="text-info">Scan the QR code with your Vaultys Wallet</span>
          )}
          {phase === "unauthorized" && (
          ...
          {phase === "error" && (
            <span className="text-danger">{errorMsg || "Authentication failed."}</span>
          )}
        </div>
        ...
        {(phase === "error" || phase === "unauthorized") && (
          <button
            ...
            className="px-4 py-1.5 bg-canvas-overlay border border-border rounded-md text-fg text-xs hover:bg-border transition-colors"
          >
            Retry
          </button>
        )}
        ...
        {agentDid && (
          <>
            ...
            <p className="text-fg-dim text-[10px] text-center break-all">
              Agent:{" "}
