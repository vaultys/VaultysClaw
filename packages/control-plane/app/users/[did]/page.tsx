              ...
              >
                ...
                {supervisorOptions.map((u) => (
                  <option key={u.did} value={u.did}>
                    {u.name ?? shortDid(u.did)}{u.isOwner ? " (Owner)" : ""}
                  </option>
                ))}
              </select>
            )}
...
function AccessTab({
  ...
  const handleAdminToggle = async (makeAdmin: boolean) => {
    ...
    setAdminError(null);
    const res = await fetch(`/api/users/${encodeURIComponent(user.did)}/admin`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
function DetailsTab({ user, onNodeClick }: { user: UserDetail; onNodeClick: (node: GraphNode) => void }) {
  return (
    <div className="space-y-6">
      <section>
...
function DangerZone({ did, userName }: { did: string; userName: string | null }) {
  const router = useRouter();
  const [removing, setRemoving] = useState(false);

  const handleRemove = async () => {
    if (!confirm(`Remove ${userName ?? shortDid(did)} and all their grants? This cannot be undone.`)) return;
    setRemoving(true);
