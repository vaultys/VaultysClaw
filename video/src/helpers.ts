...
export function useSlideUp(startFrame: number, durationFrames = 25, distance = 40) {
  const frame = useCurrentFrame();
...
export function useScaleIn(startFrame: number) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const progress = spring({ frame: frame - startFrame, fps, config: { damping: 16, stiffness: 100 } });
...
export function useFadeOut(totalFrames: number, durationFrames = 20) {
  const frame = useCurrentFrame();
  return interpolate(frame, [totalFrames - durationFrames, totalFrames], [1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
}
...
export function useSceneOpacity(totalFrames: number, fadeInFrames = 20, fadeOutFrames = 20) {
