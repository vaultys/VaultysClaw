...
function MessageBubble({ msg }: { msg: ChatMessage }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words prose prose-sm prose-invert max-w-none ${
            isUser
              ? "bg-primary-600/25 text-foreground border border-primary-500/20 rounded-tr-sm prose-headings:text-primary-300 prose-p:m-0 prose-ul:m-0 prose-ol:m-0 prose-li:m-0 prose-code:text-primary-200 prose-code:bg-primary-950/30 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-primary-950/30 prose-pre:border prose-pre:border-primary-500/20 prose-pre:text-primary-100"
          ...
          {msg.content ? (
            <ReactMarkdown
              components={{
                ...
                h3: ({ children }) => (
                  <h3 className="text-xs font-bold mt-1 mb-0.5">{children}</h3>
                ),
                blockquote: ({ children }) => (
                  <blockquote
                    className={`pl-2 border-l-2 my-1 ${
                      isUser ? "border-primary-500/50" : "border-neutral-200"
                    }`}
                  >
                    {children}
                  </blockquote>
                ...
                a: ({ children, href }) => (
                  <a
                    ...
                    className={
                      isUser
                        ? "text-primary-300 underline hover:text-primary-200"
                        : "text-primary-400 underline hover:text-primary-300"
                    }
                    target="_blank"
            ...
            <span className="inline-flex gap-1 items-center">
              <span
                className="w-1.5 h-1.5 bg-foreground-500 rounded-full animate-bounce"
...
function ToolCallCard({ event }: { event: ToolCallEvent }) {
  ...
  return (
    <div className="flex gap-3 animate-slide-up">
      ...
      <div
        className={`flex-1 max-w-[80%] rounded-xl border text-xs font-mono overflow-hidden transition-colors ${
          done
      ...
      >
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${done ? "bg-success-400" : "bg-success-400 animate-pulse"}`}
          />
          <span className="text-[10px] text-foreground-400 uppercase tracking-widest">
            Tool
          </span>
          <code className="text-success-400 font-semibold">
            {event.toolName}
          </code>
          {!done && (
                  ...
                  {typeof event.result === "string" ? (
                    <ReactMarkdown
                      components={{
                        ...
                        code: ({ children }) => (
                          <code className="px-1 py-0.5 rounded text-xs font-mono bg-success-950/30 text-success-200">
                            {children}
                          </code>
                        ),
                        pre: ({ children }) => (
                          <pre className="p-2 rounded text-xs overflow-x-auto my-1 border bg-success-950/30 border-success-500/20 text-success-100">
                            {children}
                          </pre>
                        ),
                      }}
  ...
  return (
    <div className="w-52 flex-shrink-0 flex flex-col bg-background-100 border-r border-neutral-200 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-neutral-200">
        ...
        <button
          onClick={onNew}
          className="flex items-center gap-1 text-[11px] text-primary-400 hover:text-primary-300 transition-colors font-medium"
          title="New chat"
        >
          <svg
            className="w-3 h-3"
          ...
          return (
            ...
            >
              <div className="px-3 py-2">
                <p
                  className={`text-xs leading-snug truncate font-medium ${isActive ? "text-foreground" : "text-foreground-700"}`}
                >
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  {s.source !== "web" && (
                    <>
                      <span className="text-[9px] text-neutral-300">·</span>
                      <span className="text-[9px] text-primary-400/70">
                        {s.source}
                      </span>
                    </>
                  )}
              ...
              {/* Delete button */}
              <button
                className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity w-5 h-5 flex items-center justify-center rounded text-foreground-400 hover:text-danger-400 hover:bg-danger-500/10"
                onClick={(e) => {
                  e.stopPropagation();
                  onDelete(s.id);
                }}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 text-foreground-400 px-6">
      <div className="w-14 h-14 rounded-2xl bg-background-200 border border-neutral-200 flex items-center justify-center">
        <svg
          className="w-7 h-7 text-foreground-500"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth={1.5}
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-foreground-400">
          Send a message below. Tool calls will appear inline.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-1 h-full overflow-hidden bg-background">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-background-100 border-b border-neutral-200 flex-shrink-0">
            ...
            <span className="text-sm font-medium text-foreground">
              {activeSessionId ? (
                <span className="text-foreground-700">
                  Session{" "}
                  <code className="text-[11px] text-foreground-400 font-mono">
          ...
          {items.length > 0 && (
            <button
              ...
              className="text-xs text-foreground-400 hover:text-danger-400 transition-colors flex items-center gap-1"
            >
              <svg
                className="w-3 h-3"
                viewBox="0 0 24 24"
        ...
        {/* Error banner */}
        {error && (
          <div className="mx-4 mb-2 px-3 py-2 bg-danger-500/10 border border-danger-500/25 rounded-lg text-xs text-danger-400 flex items-center gap-2">
            <svg
        ...
        <div className="px-4 pb-4 pt-2 flex-shrink-0">
          <div className="flex items-end gap-2 bg-background-200 border border-neutral-200 rounded-xl px-3 py-2.5 focus-within:border-primary-500/50 transition-colors">
            <textarea
              ref={inputRef}
              rows={1}
              value={input}
            ...
            <button
              ...
              className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-primary-600 text-white disabled:opacity-35 disabled:cursor-not-allowed hover:bg-primary-500 transition-colors"
              title="Send (Enter)"
            >
              {isStreaming ? (
                <svg
